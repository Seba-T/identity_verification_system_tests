...
fn compute_totp(key: &[u8]) -> Vec<u8> {
    let current_time = chrono::offset::Utc::now();
    let current_minute = current_time.minute();
    let rounded_minute = current_minute - current_minute % 10;
    let current_time_rounded = current_time.with_minute(rounded_minute).unwrap();
...
fn test() {
    ...
    assert!(memcmp::eq(&alice_1, &bob_1));
    assert!(memcmp::eq(&alice_2, &bob_2))
}
